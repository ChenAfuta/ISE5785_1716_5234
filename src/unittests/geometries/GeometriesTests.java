...
public class GeometriesTests {
    ...
    public void testSomeIntersections() {
        Geometries geometries = new Geometries(
                new Sphere(1, new Point(2, 0, 0)),
                new Plane(new Point(0, 0, 0.5), new Vector(0, 0, 1)),
                new Triangle(new Point(10, 0, 1), new Point(12, 2, 1), new Point(10, 2, 3)) // שלוש נקודות שונות
        );
        Ray ray = new Ray(new Point(0, 0, -1), new Vector(0, 0, 1)); // ודא שהקרן מכוונת נכון
        List<Point> intersections = geometries.findIntersections(ray);
        assertEquals(3, intersections.size(), "Expected 3 intersections with sphere and plane");
    }
