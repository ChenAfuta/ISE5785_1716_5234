...
public class PointLight extends Light implements LightSource {
    ...
    public Point getSamplePoint(Point p) {
        // הגנה: אם אין רדיוס או שהנקודה זהה למקור האור – מחזירים את מקור האור
        if (isZero(radius) || p.equals(position)) {
            return position;
        }

        Vector toP;
        try {
            toP = p.subtract(position).normalize();
        } catch (IllegalArgumentException | ArithmeticException e) {
            // Fallback במידה ונכשל נירמול (בגלל וקטור אפס)
            return position;
        }
