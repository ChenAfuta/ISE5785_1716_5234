// renderer/HouseSceneNoThreadsTest.java
package renderer;
...
public class HouseSceneNoThreadsTest {

    private final Scene scene = new Scene("House Scene No Thread Test");
    private final Camera.Builder cameraBuilder = Camera.getBuilder()
            .setRayTracer(scene, RayTracerType.SIMPLE);

    @Test
    public void houseSceneFullAcceleration() {
        ...
        for (double zPos : leftTreeZ)
            createTree(geometries, new Point(-60, 0.01, zPos), 1.0 + Math.random() * 0.3);

        double[] rightTreeZ = {-70, -110, -150, -190, -230, -270, -310};
        for (double zPos : rightTreeZ)
            createTree(geometries, new Point(70, 0.01, zPos), 0.8 + Math.random() * 0.4);
        ...
        scene.setGeometries(new Geometries(geometries.toArray(new Intersectable[0])));
        ...
        Camera camera = cameraBuilder
                ...
                .build();
        camera.setSamplingConfig(new SamplingConfig(81, TargetShape.RECTANGLE, SamplingPattern.GRID));
        ...
        camera.writeToImage("HouseNoThreads");
    }
